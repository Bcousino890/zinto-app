(function() {
  ...
  fetch(API + '/api/webchat/widget.html')
    ...
    .then(html => {
      win.innerHTML = html;
      setupEventListeners();
      if (Object.keys(i18nStrings).length) {
        applyTranslations();
        if (Object.keys(config).length) applyConfig();
      }
    });
  ...
  function applyTranslations() {
    ...
    const subtitleEl = win.querySelector('.pc-header-subtitle');
    if (subtitleEl) {
      const dot = subtitleEl.querySelector('.pc-status-dot');
      subtitleEl.textContent = '';
      if (dot) subtitleEl.appendChild(dot);
      subtitleEl.appendChild(document.createTextNode(t('header.subtitle')));
    }
    const prechatTitle = win.querySelector('#pc-prechat h4');
    const prechatDesc = win.querySelector('.pc-prechat-desc');
    const nameLabel = win.querySelector('#pc-prechat .pc-prechat-fields label:nth-of-type(1)');
    const phoneLabel = win.querySelector('#pc-prechat .pc-prechat-fields label:nth-of-type(2)');
    const emailLabel = win.querySelector('#pc-prechat-email-label');
    const nameInput = win.querySelector('#pc-prechat-name');
    const phoneInput = win.querySelector('#pc-prechat-phone');
    const emailInput = win.querySelector('#pc-prechat-email');
    ...
    if (prechatTitle) prechatTitle.textContent = t('prechat.title');
    if (prechatDesc) prechatDesc.textContent = t('prechat.description');
    if (nameLabel) nameLabel.textContent = t('prechat.label.name');
    if (phoneLabel) phoneLabel.textContent = t('prechat.label.phone');
    if (emailLabel) emailLabel.textContent = t('prechat.label.email');
    if (nameInput) nameInput.placeholder = t('prechat.placeholder.name');
    if (phoneInput) phoneInput.placeholder = t('prechat.placeholder.phone');
    if (emailInput) emailInput.placeholder = t('prechat.placeholder.email');
    if (startBtn) startBtn.textContent = t('prechat.start');
    if (elInput) elInput.placeholder = t('input.placeholder');
    if (elEmoji) elEmoji.title = t('input.addEmoji');
    if (elAttach) elAttach.title = t('input.attachFile');
    if (elSend) elSend.setAttribute('aria-label', t('input.sendMessage'));

    if (titleEl) {
      if (config.companyName) {
        titleEl.textContent = t('header.titleWithCompany', { company: config.companyName });
      } else {
  ...
  function applyConfig() {
    ...
    themeStyle.textContent = `
      ...
      #pc-emoji-picker emoji-picker{--indicator-color:${theme}; --outline-color:${theme};}
    `;
  ...
  function resolveConfiguredTheme() {
    const configuredTheme = config.theme === 'light' || config.theme === 'dark' ? config.theme : 'auto';
    if (configuredTheme !== 'auto') return configuredTheme;
    return window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches ? 'dark' : 'light';
  }

  function applyThemeAttributes() {
    const effectiveTheme = resolveConfiguredTheme();
    [btn, win, win.querySelector('#pc-emoji-picker')].forEach((el) => {
      if (el) el.setAttribute('data-pc-theme', effectiveTheme);
    });
  }

  function setupWidgetTheme() {
    if (themeMediaQuery && themeMediaListener && themeMediaQuery.removeEventListener) {
      themeMediaQuery.removeEventListener('change', themeMediaListener);
    } else if (themeMediaQuery && themeMediaListener && themeMediaQuery.removeListener) {
    ...
    if (configuredTheme === 'auto' && window.matchMedia) {
      ...
      themeMediaListener = () => applyThemeAttributes();
  ...
  function setupEventListeners() {
    ...
    if (elEmoji && elEmojiPicker) {
      ...
      if (emojiPickerLoadFailed) {
        ...
        elEmoji.title = t('emoji.unavailable');
        elEmoji.onclick = () => {
    ...
    if (dropZone) {
      dropZone.ondragover = (e) => {
        e.preventDefault();
        dropZone.style.background = 'var(--pc-msgs-bg-hover)';
      };
      dropZone.ondragleave = () => {
        dropZone.style.background = 'var(--pc-msgs-bg)';
      };
      dropZone.ondrop = (e) => {
        e.preventDefault();
        dropZone.style.background = 'var(--pc-msgs-bg)';
    ...
    if (file.type.startsWith('image/')) {
      ...
      elPreviewFile.innerHTML = `
        <div style="display: flex; align-items: center; gap: 8px;">
          ...
          <div>
            <div style="font-weight: 600;">${t('media.video')}</div>
      ...
      elPreviewFile.innerHTML = `
        <div style="display: flex; align-items: center; gap: 8px;">
          <span style="font-size: 24px;">🎵</span>
          <div>
            <div style="font-weight: 600;">${t('media.audio')}</div>
            <div style="font-size: 11px; opacity: 0.8;">${file.name}</div>
          </div>
        </div>
      `;
      ...
      elPreviewFile.innerHTML = `
        <div style="display: flex; align-items: center; gap: 8px;">
          <span style="font-size: 24px;">📄</span>
          <div>
            <div style="font-weight: 600;">${t('media.pdf')}</div>
            <div style="font-size: 11px; opacity: 0.8;">${file.name}</div>
          </div>
        </div>
      `;
      ...
      elPreviewFile.innerHTML = `
        <div style="display: flex; align-items: center; gap: 8px;">
          <span style="font-size: 24px;">📎</span>
          <div>
            <div style="font-weight: 600;">${t('media.document')}</div>
            <div style="font-size: 11px; opacity: 0.8;">${file.name}</div>
          </div>
        </div>
      `;
  ...
  function showPrechatForm() {
    return new Promise((resolve) => {
      ...
      errorEl.classList.remove('pc-prechat-notice');
      ...
      const submit = async () => {
        ...
        errorEl.classList.remove('pc-prechat-notice');
        if (!name) { errorEl.textContent = t('validation.nameRequired'); errorEl.style.display = 'block'; return; }
        if (!phone || phone.replace(/\D/g,'').length < 6) { errorEl.textContent = t('validation.phoneRequired'); errorEl.style.display = 'block'; return; }
        let displayName = name;
        try {
          const savedProfile = JSON.parse(localStorage.getItem(PROFILE_KEY) || 'null');
          const check = await fetch(API + '/api/webchat/check-phone', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
          ...
          if (check.ok) {
            ...
            if (result.exists) {
              if ((!savedProfile || !savedProfile.name) && result.name && nameEl) {
                nameEl.value = result.name;
                displayName = result.name;
              }
              errorEl.textContent = t('validation.welcomeBack');
              errorEl.classList.add('pc-prechat-notice');
              errorEl.style.display = 'block';
            }
          }
        } catch (e) {
          console.warn('WebChat widget: phone pre-check failed', e);
        }
        const profile = { name: displayName, phone, email };
  ...
  function push(type, content, msgId, mediaUrl, fileType, timestamp) {
    const elMsgs = win.querySelector('#pc-messages');
    const msgIdKey = msgId != null && msgId !== '' ? String(msgId) : '';
    if (msgIdKey && shownMsgIds.has(msgIdKey)) return null;
    ...
    if (mediaUrl) {
      ...
      if (isImage) {
        ...
        pdfDiv.innerHTML = `
          ...
          <div class=\"pc-msg-file-info\">
            <div style=\"font-weight: 600;\">${t('media.pdf')}</div>
            <div style=\"font-size: 11px; opacity: 0.8;\">${t('media.clickToView')}</div>
          </div>
        `;
        ...
        fileIcon.className = 'pc-msg-file-icon';
        fileIcon.textContent = '📎';
        const fileInfo = document.createElement('div');
        fileInfo.className = 'pc-msg-file-info';
        const fileTitle = document.createElement('div');
        const cap = content || t('media.attachment');
        if (typeof WebchatLinkify !== 'undefined' && WebchatLinkify.appendRichText) {
          WebchatLinkify.appendRichText(fileTitle, cap);
        } else {
          fileTitle.appendChild(document.createTextNode(cap));
        }
      ...
      if (content) {
        ...
        if (typeof WebchatLinkify !== 'undefined' && WebchatLinkify.appendRichText) {
          WebchatLinkify.appendRichText(captionDiv, content);
        } else {
          captionDiv.appendChild(document.createTextNode(content));
        }
        div.appendChild(captionDiv);
      }
    } else if (content) {
      if (typeof WebchatLinkify !== 'undefined' && WebchatLinkify.appendRichText) {
        WebchatLinkify.appendRichText(div, content);
      } else {
    ...
    if (msgIdKey) shownMsgIds.add(msgIdKey);
    return div;
  }
